...
const OrderTable = () => {
  ...
  return (
    <>
      {showAddModal && <AddOrderModal onClose={handleCloseAddModal} />}
      {showEditModal && selectedProduct && (
        <EditOrderModal product={selectedProduct} onClose={handleCloseEditModal} />
      )}
      <motion.div
      ...
      >
        <div className="flex flex-col mb-6">
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex items-center space-x-2">
              <Button variant="primary" label="Add Product" onClick={handleOpenAddModal} />
              <Button variant="outline" label="Filters" onClick={() => console.log("Filter clicked!")} />
            </div>
          </div>
              ...
              <tbody className="divide-y divide-gray-400">
                {currentItems.map((product) => (
                  <motion.tr key={product.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-700">{product.name}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-500">{product.manufacturing_date}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-500">{product.expiry_date}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-500">{product.stock}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-500">{product.buying_price}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-500">{product.selling_price}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">{getAvailabilityLabel(product.stock, 100)}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <Button variant="outline" label="Edit" onClick={() => handleOpenEditModal(product)} />
                    </td>
                  </motion.tr>
                ))}
              </tbody>
        ...
        <div className="flex justify-between items-center mt-6">
          <button onClick={goToPreviousPage} disabled={currentPage === 1} className="px-4 py-2 bg-gray-200 text-gray-700 rounded disabled:opacity-50">
            Previous
          </button>
          <span className="text-sm text-gray-700">
            Page {currentPage} of {totalPages}
          </span>
          <button onClick={goToNextPage} disabled={currentPage === totalPages} className="px-4 py-2 bg-gray-200 text-gray-700 rounded disabled:opacity-50">
            Next
          </button>
        </div>
      </motion.div>
